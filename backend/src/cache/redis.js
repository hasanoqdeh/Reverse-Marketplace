...
const redisClient = {
  ...
  async cacheRequest(requestId, data, ttlSeconds = config.redis.ttl.requestCache) {
    return this.setJSON(`request:${requestId}`, data, ttlSeconds);
  },

  async getCachedRequest(requestId) {
    return this.getJSON(`request:${requestId}`);
  },

  async invalidateRequest(requestId) {
    return this.del(`request:${requestId}`);
  },

  async cacheSearchResults(cacheKey, data, ttlSeconds = config.redis.ttl.searchCache) {
    return this.setJSON(`search:${cacheKey}`, data, ttlSeconds);
  },

  async getCachedSearchResults(cacheKey) {
    return this.getJSON(`search:${cacheKey}`);
  },

  async cacheCategories(data, ttlSeconds = config.redis.ttl.categoryCache) {
    return this.setJSON('categories:all', data, ttlSeconds);
  },

  async getCachedCategories() {
    return this.getJSON('categories:all');
  },
